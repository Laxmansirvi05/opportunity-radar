...
import { InterestsSection as InterestsSectionData } from "@/features/resume-toolkit/lib/schema/resume/data";
import { SectionProps } from "../types";

export function InterestsSection({ data }: SectionProps<InterestsSectionData>) {
  if (data.hidden || !data.items || data.items.length === 0) return null;

  return (
    <section className="resume-section interests-section flex flex-col gap-3">
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-2">
        {data.items.map((item) => {
          ...
          return (
            <div key={item.id} className="flex flex-col mb-1 break-inside-avoid">
