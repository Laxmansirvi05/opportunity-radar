/*
 * Vendored from LiveKit's agents-ui shadcn registry:
 *   npx shadcn@latest add @agents-ui/agent-audio-visualizer-aura
 *
 * Left byte-for-byte as published so a future `add` can update it cleanly.
 * The rule below is disabled rather than fixed: the setState call it flags is
 * how upstream drives the shader's speed uniform off agent state, and
 * rewriting it here would fork the file for a lint preference.
 */
