...
const DEFAULT_MARKERS = [
  { lat: 37.78, lng: -122.42, label: "Jobs" },
  { lat: 51.51, lng: -0.13, label: "Internships" },
  { lat: 35.68, lng: 139.69, label: "Hackathons" },
  { lat: -33.87, lng: 151.21, label: "Competitions" },
  { lat: 1.35, lng: 103.82 },
  { lat: 55.76, lng: 37.62 },
  { lat: -23.55, lng: -46.63 },
  { lat: 19.43, lng: -99.13 },
...
export function Component({
  ...
  const draw = useCallback(() => {
    ...
    for (let i = 0; i < dots.length; i++) {
      ...
      const dotSize = 1.5 + depthAlpha * 1.5;
    ...
    for (const conn of connections) {
      ...
      let [x1, y1, z1] = latLngToXYZ(
        lat1,
        lng1,
        radius
      );

      let [x2, y2, z2] = latLngToXYZ(
        lat2,
        lng2,
        radius
      );
    ...
    for (const marker of markers) {
      ...
      if (marker.label) {
        ...
        const paddingX = 10;
        const boxWidth = textWidth + paddingX * 2 + 12; // space for dot
        const boxHeight = 28;
        const boxX = sx + 12;
        ...
        ctx.fillStyle = markerColor;
        ctx.beginPath();
        ctx.arc(boxX + 12, boxY + boxHeight / 2, 3, 0, Math.PI * 2);
        ctx.fill();

        // Text
        ctx.fillStyle = "#1e293b";
        ctx.fillText(marker.label, boxX + 22, boxY + boxHeight / 2 + 4);
