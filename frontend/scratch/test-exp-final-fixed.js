...
(async () => {
  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      for (const btn of container.querySelectorAll('button[type="button"]')) {
    ...
    await new Promise(r => setTimeout(r, 2000));
    await (await page.$('canvas')).screenshot({path: 'exp-1-edit.png'});

    // 2. DUPLICATE
    console.log("2. DUPLICATE...");
    await page.evaluate(() => {
      document.querySelectorAll('#sidebar-experience button[aria-haspopup="menu"]')[0].click();
    });
    await new Promise(r => setTimeout(r, 500));
    await page.evaluate(() => {
      [...document.querySelectorAll('[role="menuitem"]')].find(mi => mi.innerText.includes('Duplicate')).click();
    });
    await new Promise(r => setTimeout(r, 1000)); // Dialog opens
    await page.evaluate(() => {
      [...document.querySelectorAll('[role="dialog"] button')].find(b => b.innerText.includes('Create')).click();
    });
    await new Promise(r => setTimeout(r, 2000));
    await (await page.$('canvas')).screenshot({path: 'exp-2-duplicate.png'});
    ...
    await new Promise(r => setTimeout(r, 2000));
    await (await page.$('canvas')).screenshot({path: 'exp-3-add.png'});

    // 4. DELETE
    console.log("4. DELETE...");
    await page.evaluate(() => {
      const menus = document.querySelectorAll('#sidebar-experience button[aria-haspopup="menu"]');
      menus[menus.length - 1].click(); // Delete the last one we just added
    });
    await new Promise(r => setTimeout(r, 500));
    await page.evaluate(() => {
      [...document.querySelectorAll('[role="menuitem"]')].find(mi => mi.innerText.includes('Delete')).click();
    });
    await new Promise(r => setTimeout(r, 500)); // Confirm dialog opens
    await page.evaluate(() => {
    ...
    await new Promise(r => setTimeout(r, 2000));
    await (await page.$('canvas')).screenshot({path: 'exp-4-delete.png'});
