...
(async () => {
  ...
  try {
    ...
    console.log("1. EDIT...");
    await page.evaluate(() => {
      for (const btn of document.querySelectorAll('#sidebar-education button[type="button"]')) {
    ...
    await page.evaluate(() => {
      const input = document.querySelector('[role="dialog"] input[name="institution"]');
      if (input) {
    ...
    await page.evaluate(() => {
      [...document.querySelectorAll('[role="dialog"] button')].find(b => b.type === 'submit' || b.innerText.includes('Save')).click();
    });
    await new Promise(r => setTimeout(r, 2000));
    ...
    await new Promise(r => setTimeout(r, 1500)); 
    await page.evaluate(() => {
      [...document.querySelectorAll('[role="dialog"] button')].find(b => b.type === 'submit' || b.innerText.includes('Create')).click();
    });
    await new Promise(r => setTimeout(r, 2000));
