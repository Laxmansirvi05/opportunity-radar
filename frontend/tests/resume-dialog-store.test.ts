/**
 * Moved here from features/resume/dialogs/store.test.ts, where it never ran:
 * the vitest include globs only collect .test.tsx files under features/, and
 * this is a .ts file, so nothing picked it up. It had also gone stale — it
 * asserted the "auth" and "api-key" dialog domains that were dropped when the
 * toolkit was integrated, and drove every case with an "api-key.create"
 * dialog that no longer exists in the schema union. Both are corrected here,
 * and the store itself is real, live code worth covering.
 */
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import { dialogSchemaRegistries, dialogTypeSchema } from "@/features/resume/dialogs/schemas";
...
describe("useDialogStore", () => {
	...
	describe("openDialog", () => {
		it("uses schema entries from domain registries", () => {
			expect(dialogSchemaRegistries.map((registry) => registry.domain)).toEqual(["resume"]);
